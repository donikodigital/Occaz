...
/**
 * Un seul rafraîchissement en vol à la fois — même principe que le
 * client mobile (mobile/src/services/api/client.ts), voir sa note pour
 * le pourquoi.
 */
...
export interface RequestOptions {
  ...
  /** false pour les routes publiques (login, OTP...) — n'essaie pas d'ajouter un jeton. */
  auth?: boolean;
}
...
/** Point d'entrée unique pour tout appel réseau — même contrat que le client mobile. */
export async function apiRequest<T>(path: string, options: RequestOptions = {}): Promise<T> {
  const useAuth = options.auth !== false;
  const accessToken = useAuth ? tokenStorage.getAccessToken() : null;
